...
@section('page')
                                    ...
                                    @foreach ($courseSchedules as $schedule)
                                        <tr id="course-{{ $course->kode_mk }}" class="course-item">
                                            ...
                                            <td class="p-2 border text-center">
                                                <button
                                                    onclick="showCourseModal('{{ $course->kode_mk }}', '{{ $course->nama_mk }}', {{ $course->sks }}, '{{ $schedule->hari }}', '{{ $schedule->jam_mulai }}', '{{ $schedule->jam_selesai }}', '{{ $schedule->ruang }}', {{ $schedule->kapasitas }})"
                                                    class="bg-blue-500 text-white px-2 py-1 rounded">Pilih</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                @endforeach
    ...
    <script>
        ...
        function updateSelectedCourses() {
            ...
            document.getElementById('sksIndicator').innerHTML =
                `<button class="bg-blue-500 text-white p-3 rounded-lg">${totalSKS} SKS Dipilih</button>`;
        }
